...
ESX.RegisterServerCallback("cokeRun:checkIfHasKey", function(source, cb)
    ...
    local hasKey = xPlayer.getInventoryItem(Config.items.planeKey)
    if hasKey.count > 0 then
        cb(true)
    else
        cb(false)
    end
...
function startTimer(playerId)
    Citizen.Wait(Config.missionTime * 60000)
    if missionInProgress then
        missionInProgress = false
        TriggerClientEvent("cokeRun:missionFailed", playerId)
    end
end